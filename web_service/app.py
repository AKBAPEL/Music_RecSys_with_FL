...
def eda_demonstration(train: pd.DataFrame) -> None:
    ...
    for i in range(0, len(unique_tabs), 2):
        sns.histplot(
            data=train[train["source_type"].isin(unique_tabs[i : i + 2])],
            x="source_type",
            hue="target",
            multiple="dodge",
            shrink=0.9,
...
with st.expander("Управление моделями"):
    ...
    model_type = st.selectbox(
        "Model Type", ["ALS"]
    )  # добавить может быть Matrix Factorization
    params = {
    ...
    if st.button("Recommend"):
        response = requests.post(
            f"{API_URL}/predict", params={"user_id": user_id, "n": n}
        )
